<template>
  <div class="media-input">
    ...
    <template v-if="type === 'video'">
      ...
      <div class="btns">
        <Button class="primary-btn-mini" @click="emit('close')" style="margin-right: 10px;">取消</Button>
        <Button class="primary-btn-mini" type="primary" @click="insertVideo()">确认</Button>
      </div>
    </template>
    ...
    <template v-if="type === 'audio'">
      ...
      <div class="btns">
        <Button class="primary-btn-mini" @click="emit('close')" style="margin-right: 10px;">取消</Button>
        <Button class="primary-btn-mini" type="primary" @click="insertAudio()">确认</Button>
      </div>
    </template>
  </div>
</template>
...
const insertVideo = () => {
  if (!videoSrc.value) return message.warning('请输入正确的视频地址')
  emit('insertVideo', videoSrc.value)
}

const insertAudio = () => {
  if (!audioSrc.value) return message.warning('请输入正确的音频地址')
